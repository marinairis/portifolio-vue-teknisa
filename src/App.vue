<template>
  <div id="app">
    <HeaderComponent
      @changeValue="newSearchValue"
      @changeLinguagem="newLinguagemValue"
      @changeModo="newModoValue"
    ></HeaderComponent>
    ...
    getFiltredDevs: function () {
      ...
      escopo.filtredDevs = devs.filter((dev) => {
        ...
        let nomesBatendo =
          !escopo.search ||
          escopo
            .getFilterCharacters(dev.name)
            .indexOf(escopo.getFilterCharacters(escopo.search)) >= 0; //se um dos dois forem verdade eu recebo TRUE
        let linguagensBatendo = dev.programmingLanguages.filter(
          (programming) =>
            escopo.linguagem.indexOf(
              escopo.getFilterCharacters(programming.id)
            ) >= 0
        );
        ...
        if (escopo.modo === "e") {
          if (
            semFiltro ||
            (nomesBatendo &&
              linguagensBatendo.length === escopo.linguagem.length)
          ) {
            return true;
          }
          return false;
          ...
          if (
            semFiltro ||
            (nomesBatendo &&
              (escopo.linguagem.length === 0 || linguagensBatendo.length >= 1))
          ) {
            return true;
          }
          return false;
